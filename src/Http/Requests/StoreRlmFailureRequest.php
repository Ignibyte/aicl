<?php

namespace Aicl\Http\Requests;

use Aicl\Enums\FailureCategory;
use Aicl\Enums\FailureSeverity;
use Aicl\Models\RlmFailure;
use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Validation\Rule;

class StoreRlmFailureRequest extends FormRequest
{
    public function authorize(): bool
    {
        return $this->user()->can('create', RlmFailure::class);
    }

    /**
     * @return array<string, mixed>
     */
    public function rules(): array
    {
        return [
            'failure_code' => ['required', 'string', 'max:255', 'unique:rlm_failures,failure_code'],
            'pattern_id' => ['nullable', 'string', 'max:255'],
            'category' => ['required', 'string', Rule::enum(FailureCategory::class)],
            'subcategory' => ['nullable', 'string', 'max:255'],
            'title' => ['required', 'string', 'max:255'],
            'description' => ['required', 'string'],
            'root_cause' => ['nullable', 'string'],
            'fix' => ['nullable', 'string'],
            'preventive_rule' => ['nullable', 'string'],
            'severity' => ['required', 'string', Rule::enum(FailureSeverity::class)],
            'entity_context' => ['nullable', 'array'],
            'scaffolding_fixed' => ['boolean'],
            'first_seen_at' => ['nullable', 'date'],
            'last_seen_at' => ['nullable', 'date'],
            'aicl_version' => ['nullable', 'string', 'max:255'],
            'laravel_version' => ['nullable', 'string', 'max:255'],
            'status' => ['nullable', 'string'],
            'is_active' => ['boolean'],
        ];
    }

    /**
     * @return array<string, string>
     */
    public function messages(): array
    {
        return [];
    }
}
